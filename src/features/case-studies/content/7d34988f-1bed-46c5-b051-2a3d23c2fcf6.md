...
The goal of this case study is to illustrate the application of design patterns in production code, as well as my ability to design and build scalable, maintainable systems, using a real-life example from my past work experience. This case study will focus on the application of these specific design patterns and the problems they are intended to solve, including a working code example. I'll also include high-level technical specifics around the automation to help provide context.
...
1. **Gather Data**: The automation begins by gathering data from the **ServiceNow API** on hardware incidents submitted by users. These users are distributed all across North America (in the USA and Canada). Each incident includes information on the user's geographic location as well as the specific hardware affected by or causing the incident. For example, an incident could be submitted by a user in Seattle, WA for their malfunctioning credit card reader. The automation then queries Splunk for events and logs forwarded by the affected hardware - leveraging the **Splunk REST API** - to create a "snapshot" of the device's health over the past 7 days.
2. **Analyze**: The device snapshot(s) are analyzed to try and determine what might be wrong. We worked with the product team (our customer) to map out what data and patterns in the device snapshot indicates various anomalous conditions and how to respond. Analysis was essentially iterating over these known anomalous conditions and potentially finding a match.
3. **Take Action**: With analysis complete, our automation would execute some action intended to either remedy the device remotely, or send the incident to a designated team for resolution. The specific action to take is entirely dependent on the diagnosis of the device in the previous step.
   - Note: the automation may not be able to pinpoint any issue with the device, due to it being limited to a) the data sourced from Splunk and b) the hard-coded conditions for which it is programmed to check for in the device snapshot(s). In these cases, the automation does not take direct action to fix the device, but it always does _something_, such as providing data in the incident work notes to aid an analyst.

## Problems

During initial development, we weren't thinking about optimal designs, and even the above high-level steps weren't yet apparent to us, but this wasn't necessarily an oversight or mistake. An important design philosophy (attributed to software engineer Kent Beck) perfectly summarizes the situation we found ourselves in: **"Make it work, make it right, make it fast."** Business requirements - such as deadlines - come with the territory of writing software for business purposes. Therefore, our initial priority was to get something functional in production for our stakeholders (make it work), not making it perfect the first time.

As we expanded the automation's capabilities, several code smells quickly became apparent:

- **Too many responsibilities in one place leading to tightly coupled logic.** The automation was several hundred lines of code, and 95% of the code was being executed by 2 methods of a class. That's a lot of work and responsibility for 2 methods.

- **Difficult to test the code.** When trying to test a function/method that is doing too much, you have to make lots of assertions in each test and write many test cases to account for many different conditions. You end up testing too many different things in a single unit test.

- **Poorly managed complexity.** There's nothing necessarily "wrong" with complexity in computer programs, but it's important to manage that complexity well as programs grow. This complexity came from too few methods doing too many things, making the code harder to understand. Each new feature became harder to implement than the last because we weren't thinking about managing complexity through a well-thought-out design.
...
Design patterns should not be "forced", but when applied correctly, they provide elegant solutions to common problems that occur in software engineering. This is part of a concept known as **emergent design**. The Strategy and Factory design patterns were not part of my team's initial design for our workflow automation service. Once we had a functional system in place, we were able to take a look at the system from a high-level. The Strategy and Factory design patterns quickly became apparent then, but prior to development, we had no idea that these patterns would fit our system so well.

Refactoring the workflow automation did not change any existing features and functionality of the system, but future maintainance and feature implementation efforts were made substantially easier. While this example code is extremely simplified, a real production system may have much more complex logic implemented in the factory and individual strategies. These patterns are highly applicable to "rule-based" systems (or parts of a larger system) that decide what to do based on some input at runtime, and then executing that action.
