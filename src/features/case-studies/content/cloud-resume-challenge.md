...
**TL;DR: AI tools can help you learn, but it can't learn for you. It can be an effective learning tool and productivity enhancer, but never blindly trust its output, as you -- the engineer -- are ultimately responsible.**

---

One does not simply write a software engineering blog post in 2026 without mentioning artificial intelligence at least once! Sarcasm aside, I do think it's worth mentioning _how_ I used AI tools while working on my website and the Cloud Resume Challenge (especially if you are reading this and are interested in completing the challenge yourself).

First, I did make use of several AI tools while completing this project. AI, particularly Generative AI, can be extremely helpful for completing or expediting certain tasks. What's most important to me is that **I did not _rely_ solely on AI to complete the project**.

### Where I did NOT use AI:
- **Completing the Cloud Resume Challenge**: While AI can be a productivity multiplier and useful learning assistant, it can't learn things for you. Frankly, you _can_ prompt your way through most of the Cloud Resume Challenge (or building a static website). Ask an AI to produce the code you need and how to deploy it using Terraform and the AWS CLI. The complexity of the challenge is low, so I don't doubt that this is possible. But this is not going to help you the way that the challenge is meant to help you. You're shooting yourself in the foot in several ways:
  - **Vibe coding != learning.** Struggling through problems and experiencing problems first hand is part of a good learning experience. This challenge is not a checkbox to tick that will magically open doors for you. **It is meant to be a learning experience where the primary takeaway is knowledge and experience**. The outcome -- this website -- is secondary. You want to walk away from this challenge with that experience and some stories of the problems you faced. Interviewers will want to hear about these struggles and how you handled them.
  - **Decision-making**: Learning aside, AI cannot be held accountable for decisions that you make in any project. Mindlessly copy-pasting code or following steps to implement your website is irresponsible and could result in lost data or unexpectedly high cloud bills. (This is a fact regardless of whether the code came from ChatGPT or Stack Overflow). A simple script could send millions of `GET` requests per hour to a public/insecure S3 bucket, resulting in a bill worth hundreds or even thousands of dollars.
- **Cloud certification**: This is just an example that helps support the previous bullet point. AI might help you study for a certification exam, but it can't take that exam for you.
- **Content**: An area that an LLM could certainly be used is writing content, such as this blog post. It almost certainly would be able to create this blog post faster than I could by hand. And, it almost certainly would appear very obvious that the post is AI generated. What's the value in that? If you've read even part of this post, the reason was most likely to learn about the things I learned about while completing this challenge. You probably aren't interested in reading "generic Cloud Resume Challenge blog post." You would be more interested in _my_ experience with the challenge, my takeaways, advise, etc. The point is that you're hurting yourself by replacing your human experience with machine-generated writing.

### Where I did use AI:
- **Initial design**: Before starting on the frontend work, I headed over to **Figma** to wireframe a design for the website. Once I got there, I discovered (to little surprise) that Figma had a new AI feature: describe your project to the AI, and it produces a design for you.
  - There is certainly pros and cons to using an LLM to produce your design. Ask it to create a software engineer portfolio site, and you'll get a design very similar to other software engineer portfolio sites. It'll produce something standard and expected. For my goals, this was okay. I wanted an attractive landing page, not a super-unique frontend with complex animations to show off my better-than-average frontend skills.
  - Figma's AI didn't just produce a mock-up; it output the code for the website, too. But I wasn't interested in copy-pasting this output; I would need to tailor it excessively to get exactly what I wanted. My prompt didn't include much technical details (i.e., "use Material UI and TypeScript") because I was after a design, not the implementation. I used the design as a reference but implemented everything myself.
- **Discovery**: Asking Claude and ChatGPT questions on how I could implement my ideas helped me learn about existing tools quickly. For example, I knew that I wanted to write blog posts (like this one) in Markdown and render them on my website, keeping content decoupled from code. AI tools helped me discover `npm` packages like `react-markdown` and `react-syntax-highlighter` and how to use them. I was also able to weigh trade-offs of how I could and/or should implement the blog posts. For example, storing and retrieving the posts from a database vs. including the content as `.md` files on the frontend.
- **Amazon Q**: This is AWS's native LLM integrated directly into the AWS platform. This has been one of my favorite uses of Generative AI. AWS's documentation is extensive, which is great, but it can also be a lot to sift through. When I had questions about my resources and how they were set up, or troubleshooting an issue I was experiencing, Amazon Q could analyze my configuration to determine possible reasons for the issue(s) I was experiencing. It would provide steps on what to check and links to existing documentation. The output wasn't always perfect, but it was usually very close, and the information was up-to-date. Third-party LLM tools struggled to provide me with modern AWS solutions, especially because they did not have full context to my configuration.
</details>
